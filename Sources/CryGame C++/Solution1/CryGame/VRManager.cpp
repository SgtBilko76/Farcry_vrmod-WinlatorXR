...
void VRManager::CaptureEye(int eye)
{
	...
	// buffer is the full X screen but the engine only renders the eye into the top-left GetRenderSize
	// region of it, so copy just that region (a full-buffer copy would grab the unrendered rest and
	// show the scene shrunk into a corner - a picture-in-picture).
...
void VRManager::CaptureHUD()
{
	...
	// acquire and copy the current back buffer to the HUD texture (only the rendered region, as CaptureEye)
	ComPtr<IDirect3DSurface9> backBuffer;
	m_d3d->device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, backBuffer.GetAddressOf());
	ComPtr<IDirect3DSurface9> texSurface;
...
void VRManager::RegisterCVars()
{
	...
	console->Register("vr_winlatorxr_render_height", &vr_winlatorxr_render_height, 1188, VF_DUMPTODISK, "Per-eye render target height when running under WinlatorXR (Quest/Pico); width follows the headset FOV aspect. The game is CPU-bound on these devices, so a tall render (it is squeezed 2:1 into the side-by-side frame) is affordable and much sharper");
