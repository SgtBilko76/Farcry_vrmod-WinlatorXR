...
void VRManager::RegisterCVars()
{
	...
	console->Register("vr_winlatorxr_render_height", &vr_winlatorxr_render_height, 1440, VF_DUMPTODISK, "Per-eye render target height when running under WinlatorXR (Quest/Pico); width follows the headset FOV aspect. The game is CPU-bound on these devices, so a tall render (it is squeezed 2:1 into the side-by-side frame) is affordable and much sharper");
